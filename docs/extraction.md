...
## extract notice8k / notice8k-select（8-K 公告事件，独立轨）

新闻漏斗只吃 NEWS/FLASH/ARTICLE/ROBOT 四类；US_NOTICE 里的 8-K 公告走独立两步接入
（新闻覆盖不到的中小盘协议/高管变动/融资/临床数据等事件的主要来源）：

1. `extract notice8k --date YYYY-MM-DD`（src/extraction/notice_8k.py，零 LLM）：
   按附件 `file_type=="8-K"` 精确筛选（排除 8-K/A），缺 body 的从 v2 notice.jsonl 按
   accession 离线补 SEC 全文，产出精简事件行 `notice_8k/US_NOTICE.8k.<date>.jsonl`
   （schema/extraction/notice_8k_event.schema.json）。也支持 `--day N`/`--month N` 窗口。
2. `extract notice8k-select --date YYYY-MM-DD`（src/extraction/notice_8k_select.py，LLM triage）：
   逐条判定 `is_valid_event 且 significance≥3`，断点续跑落 `selected/triage_8k.jsonl`；
   护栏：同(类型,日期,主体)去重。产出 `selected/selected_8k.jsonl`，event_id 形如
   `EVT8K_<accession>`。

**跨源去重方向：8-K 是原始披露、先到，新闻让位**。`extract select`（新闻）在入选后对
已存在的 EVT8K 事件去重（select.py `dedup_vs_8k`）：共享主 symbol 且（事件日相同，或
同 event_type 且日期差≤3 天——覆盖 8-K 盘后申报、新闻 T+1 铺开的时间错位）则丢弃新闻
事件；类型不同（如财报 8-K vs 次日分析师评级）视为衍生新事件保留。**因此按日跑时先跑
notice8k-select 再跑新闻 select**。

下游全复用：`extract structure` 自动合并 selected_8k.jsonl（8-K 事件用公告自身正文作
单篇输入，额度 8400 字符，不走 members.parquet）；complete fetch/label/assemble 不变，
按 `--date` 过滤直通 final。05-29 实测：216 条 8-K → 110 入选（51%）；新闻 22 → 19
（3 个让位 8-K）→ final 共 125（106 8-K + 19 新闻）。
