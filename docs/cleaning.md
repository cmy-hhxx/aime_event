...
```bash
# 增量处理未完成的 batch
.venv/bin/python -m src.main clean

# 清空 state 后从 raw 全量重建
.venv/bin/python -m src.main clean fresh

# 不重新 ingest，仅从已有 state 重导出 output
.venv/bin/python -m src.main clean export
```
...
默认只写 cleaned 输出；如需同时写出 `duplicates/`、`rejects/` 辅助文件，运行时加
`--write-aux-outputs`。
...
**transform 步骤**（`src/cleaning/ingest/transform.py`）：orjson 解析 → HTML 转纯文本 → 字段映射（source / entities / notice / tags）→ 校验 id、content_type、时间、正文。
...
| 阶段 | 触发命令 | 核心模块 | 输入 → 输出 |
|------|----------|----------|-------------|
| Ingest | `run` / `fresh` | `cleaning/pipeline` · `cleaning/ingest/transform` · `cleaning/storage/staging` | raw NDJSON → `state/dedup.db` + `payloads/`；reject 暂存 DB |
| 精确去重 | 三者均 export | `cleaning/storage/winners` · `cleaning/dedup/exact` | `records` → `dedup_winners` + `duplicate_records`（content / id） |
| 近似去重 | 三者均 export | `cleaning/storage/winners` · `cleaning/dedup/near` | `dedup_winners` → `near_duplicate_losers` + 审计候选对 |
| 写出 | 三者均 export | `cleaning/pipeline` · `cleaning/output/views` · `common/io` | state → `cleaned/` · `duplicates/` · `rejects/` |
| 报表 | 三者均 export | `cleaning/reporting/writer` | state + output 计数 → `reports/` |
...
完整判定流程见上文 **[去重流程](#去重流程)** 三张图。实现位于 `src/cleaning/dedup/exact.py`（精确键）与 `src/cleaning/dedup/near.py`（近似合并）。
